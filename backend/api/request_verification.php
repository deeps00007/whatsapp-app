...
$url = "https://graph.facebook.com/v23.0/" . urlencode($phone_number_id) . "/request_verification_code";
...
error_log("[request_verification] user=$user_id phone_id=$phone_number_id method=$method HTTP=$http_code body=" . substr($response, 0, 300));

if ($http_code === 200) {
    echo json_encode(['success' => true, 'message' => "Verification code sent via $method to your phone number."]);
} else {
