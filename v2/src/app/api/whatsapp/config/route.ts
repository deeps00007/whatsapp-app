...
export async function GET() {
  try {
    ...
    const { data: config, error: configError } = await supabase
      ...
      .select('phone_number_id, access_token, status, coexistence_mode, coexistence_region, app_history_synced_at')
      .eq('user_id', user.id)
      .maybeSingle()
    ...
    try {
      ...
      return NextResponse.json({
        ...
        coexistence_mode: config.coexistence_mode ?? false,
        coexistence_region: config.coexistence_region ?? null,
        app_history_synced_at: config.app_history_synced_at ?? null,
      })
    } catch (err) {
...
export async function POST(request: Request) {
  try {
    ...
    const { phone_number_id, waba_id, access_token, verify_token, coexistence_mode, coexistence_region } = body
    ...
    if (existing) {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          coexistence_mode: coexistence_mode === true,
          coexistence_region: coexistence_region || null,
        })
        .eq('user_id', user.id)
