...
export function MessageThread({
  ...
  const handleAssignChange = useCallback(
    async (agentId: string | null) => {
      ...
      const res = await fetch('/api/conversations/assign', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ conversation_id: conversation.id, agent_id: agentId }),
      });
  ...
  return (
    <div className={cn("flex flex-1 flex-col", DOODLE_BG_CLASSES)}>
      {/* Header — solid bg-background sits on top of the doodle so the
          name/avatar/dropdowns stay legible. */}
      <div className="flex items-center justify-between gap-2 border-b border-border bg-background px-3 py-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-secondary text-sm font-medium text-foreground">
            ...
            {messageGroups.map((group) => (
              <div key={group.date}>
                ...
                <div className="mb-3 flex items-center justify-center">
                  <span className="rounded-full bg-card px-3 py-1 text-[10px] font-medium text-muted-foreground">
