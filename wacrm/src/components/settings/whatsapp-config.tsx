...
export function WhatsAppConfig() {
  ...
  // Coexistence (India 2026)
  const [coexistenceMode, setCoexistenceMode] = useState(false);
  const [coexistenceRegion, setCoexistenceRegion] = useState('india');
  ...
  const fetchConfig = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        setCoexistenceMode(data.coexistence_mode ?? false);
        setCoexistenceRegion(data.coexistence_region || 'india');
      } else {
        setConfig(null);
        setPhoneNumberId('');
        setWabaId('');
  ...
  async function handleSave() {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        coexistence_region: coexistenceMode ? coexistenceRegion : null,
      };
      ...
      <div className="space-y-6">
        ...
        <Alert className="bg-slate-900 border-slate-700">
          ...
          <AlertDescription className="text-slate-400">
            {connectionStatus === 'connected'
              ? coexistenceMode
                ? 'Your WhatsApp Business API is connected. Coexistence mode is active — your mobile app remains active for calls and manual chats.'
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500"
              />
              {config && verifyToken === MASKED_TOKEN && (
                <p className="text-xs text-slate-500">
                  Token is hidden for security. Click the field to enter a new one.
                </p>
              )}
              <p className="text-xs text-slate-500">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Label className="text-slate-300 font-medium">
                      Coexistence Mode (India)
                    </Label>
                    <span className="inline-flex items-center rounded-full bg-emerald-950/60 px-2 py-0.5 text-[10px] font-medium text-emerald-400 ring-1 ring-emerald-800">
                      New 2026
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Keep your WhatsApp Business App (mobile) active alongside the API.
                    Voice/video calls stay on the app. Automation, broadcasts, and
                    CRM inbox run through the API. Your previous chat history is preserved.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setCoexistenceMode((v) => !v)}
                  className={`
                    relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 transition-colors
                    ${coexistenceMode ? 'bg-primary border-primary' : 'bg-slate-700 border-slate-600'}
                  `}
                >
                  <span
                    className={`
                      inline-block size-5 rounded-full bg-white transition-transform
                      ${coexistenceMode ? 'translate-x-5' : 'translate-x-0'}
                    `}
                  />
                </button>
              </div>
                ...
                <div className="mt-3 rounded-lg bg-emerald-950/20 border border-emerald-900/40 p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-emerald-200">
                      App + API running together
                    </span>
                  </div>
                  <ul className="space-y-1.5 text-xs text-emerald-100/70">
                    <li className="flex items-start gap-1.5">
                      <Check className="size-3.5 mt-0.5 text-emerald-500 shrink-0" />
                      Mobile app stays active for calls & manual chats
                    </li>
                    <li className="flex items-start gap-1.5">
                      <Check className="size-3.5 mt-0.5 text-emerald-500 shrink-0" />
                      CRM receives real-time API messages via webhook
                    </li>
                    <li className="flex items-start gap-1.5">
                      <Check className="size-3.5 mt-0.5 text-emerald-500 shrink-0" />
                      No need to delete or migrate your existing number
                    </li>
                  </ul>
                  ...
                  <div className="mt-3">
                    ...
                    <select
                      value={coexistenceRegion}
                      onChange={(e) => setCoexistenceRegion(e.target.value)}
                      className="w-full rounded-md bg-emerald-950/40 border border-emerald-800/60 text-sm text-emerald-100 px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-emerald-600"
                    >
                      <option value="india">India (Supported)</option>
          ...
          <CardContent>
            <Accordion>
              ...
              <AccordionItem className="border-slate-700">
                <AccordionTrigger className="text-slate-300 hover:text-white hover:no-underline">
                  <span className="flex items-center gap-2">
                    <span className="flex size-5 items-center justify-center rounded-full bg-emerald-600 text-xs font-bold text-white">5</span>
                    Enable Coexistence (India)
                  </span>
                </AccordionTrigger>
                <AccordionContent className="text-slate-400">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>Toggle <strong className="text-slate-200">Coexistence Mode</strong> in the form above</li>
                    <li>Select <strong className="text-slate-200">India</strong> as your region</li>
                    <li>Save configuration — your mobile app stays active</li>
                    <li>Export your Business App chat history from your BSP or backup</li>
                    <li>Use the <strong className="text-slate-200">Import History</strong> section to migrate old chats</li>
                  </ol>
                  <p className="text-xs text-slate-500 mt-2">
                    Voice/video calls continue on the mobile app. CRM automation,
                    broadcasts, and team inbox run through the API.
                  </p>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
